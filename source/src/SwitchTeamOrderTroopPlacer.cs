...
namespace EnhancedBattleTest
{
    ...
    public class SwitchTeamOrderTroopPlacer : MissionView
    {
        ...
        public override void OnMissionTick(float dt)
        {
            ...
            if (this.Mission.PlayerTeam == null && (missionPeer == null || missionPeer.Team != this.Mission.AttackerTeam && missionPeer.Team != this.Mission.DefenderTeam))
                return;
        ...
        private void UpdateFormationDrawingForFacingOrder(bool giveOrder)
        {
            ...
            this.PlayerOrderController.SimulateNewFacingOrder(OrderController.GetOrderLookAtDirection(this.PlayerOrderController.SelectedFormations, this.MissionScreen.GetOrderFlagPosition().AsVec2), out simulationAgentFrames);
            int entityIndex = 0;
            this.HideOrderPositionEntities();
            foreach ((Agent, WorldFrame) valueTuple in simulationAgentFrames)
            {
                WorldFrame frame = valueTuple.Item2;
        ...
        private void UpdateFormationDrawingForFormingOrder(bool giveOrder)
        {
            ...
            float orderFormCustomWidth = OrderController.GetOrderFormCustomWidth(this.PlayerOrderController.SelectedFormations, origin1);
            List<(Agent, WorldFrame)> simulationAgentFrames;
            this.PlayerOrderController.SimulateNewCustomWidthOrder(orderFormCustomWidth, out simulationAgentFrames);
            Formation formation = this.PlayerOrderController.SelectedFormations.MaxBy<Formation, int>((Func<Formation, int>)(f => f.Units.Count<Agent>()));
            int entityIndex = 0;
            this.HideOrderPositionEntities();
            foreach ((Agent, WorldFrame) valueTuple in simulationAgentFrames)
            {
            ...
            for (int index = 0; index < num1; ++index)
            {
                ...
                WorldFrame frame = new WorldFrame(orderFlagFrame.rotation, origin2);
        ...
        private void UpdateFormationDrawing(bool giveOrder)
        {
            ...
            if (this.MissionScreen.MouseVisible && this._formationDrawingStartingTime.HasValue && (double)MBCommon.GetTime(MBCommon.TimeType.Mission) - (double)this._formationDrawingStartingTime.Value < 0.300000011920929)
            {
                flag = true;
                formationRealEndingPosition = this._formationDrawingStartingPosition.Value;
            }
            if (!flag)
            {
                ...
                if (!this.Mission.Scene.RayCastForClosestEntityOrTerrain(rayBegin, rayEnd, out collisionDistance, 0.3f, BodyFlags.CommonFocusRayCastExcludeFlags))
                    return;
            ...
            if ((!InputKey.LeftMouseButton.IsDown() || this._formationDrawingStartingPointOfMouse.HasValue) && this.IsDrawingAttaching)
          ...
          bool isFormationLayoutVertical)
        {
            ...
            {
                WorldPosition attachPosition = MovementOrder.GetAttachPosition(selectedFormation, this.AttachTarget, this.AttachSide);
                ...
                List<(Agent, WorldFrame)> simulationAgentFrames;
                OrderController.SimulateNewOrderWithPositionAndDirection(Enumerable.Repeat<Formation>(selectedFormation, 1), this.PlayerOrderController.simulationFormations, formationLineBegin, formationLineEnd, out simulationAgentFrames, isFormationLayoutVertical);
                foreach ((Agent, WorldFrame) valueTuple in simulationAgentFrames)
                {
                    WorldFrame frame = valueTuple.Item2;
                    this.AddOrderPositionEntity(entityIndex, ref frame, giveOrder, -1f);
            ...
            if (!giveOrder)
                return;
            this.PlayerOrderController.SetOrderWithFormationAndNumber(OrderType.Attach, this.AttachTarget, (int)this.AttachSide);
        }
            ...
            switch (this.GetCursorState())
            {
                case SwitchTeamOrderTroopPlacer.CursorState.Normal:
                    ...
                    if (this.Mission.Scene.RayCastForClosestEntityOrTerrain(rayBegin, rayEnd, out collisionDistance, 0.3f, BodyFlags.CommonFocusRayCastExcludeFlags))
                    {
                        Vec3 vec3 = rayEnd - rayBegin;
                        double num = (double)vec3.Normalize();
                        this._formationDrawingStartingPosition = new WorldPosition?(new WorldPosition(Mission.Current.Scene, UIntPtr.Zero, rayBegin + vec3 * collisionDistance, false));
                        this._formationDrawingStartingPointOfMouse = new Vec2?(this.Input.GetMousePositionPixel());
                        this._formationDrawingStartingTime = new float?(MBCommon.GetTime(MBCommon.TimeType.Mission));
                        break;
                    }
                    this._formationDrawingStartingPosition = new WorldPosition?();
                    this._formationDrawingStartingPointOfMouse = new Vec2?();
        ...
        private SwitchTeamOrderTroopPlacer.CursorState GetCursorState()
        {
            ...
            if (!this.PlayerOrderController.SelectedFormations.IsEmpty<Formation>() && this._clickedFormation == null)
            {
                ...
                if (!this.Mission.Scene.RayCastForClosestEntityOrTerrain(rayBegin, rayEnd, out collisionDistance, out collidedEntity, 0.3f, BodyFlags.CommonFocusRayCastExcludeFlags))
                    collisionDistance = 1000f;
                if (cursorState == SwitchTeamOrderTroopPlacer.CursorState.Invisible && (double)collisionDistance < 1000.0)
                {
                    ...
                    if (cursorState == SwitchTeamOrderTroopPlacer.CursorState.Invisible && this.MissionScreen.OrderFlag.FocusedOrderableObject != null)
        ...
        private void UpdateAttachData()
        {
            ...
            foreach (Formation formation in this.PlayerTeam.Formations.Where<Formation>((Func<Formation, bool>)(f => !this.PlayerOrderController.IsFormationListening(f))))
            {
                WorldPosition worldPosition;
                Vec2 asVec2;
                if (this.AttachTarget != null)
          ...
          float alpha = -1f)
        {
            while (this._orderPositionEntities.Count <= entityIndex)
            {
                ...
                if ((NativeObject)SwitchTeamOrderTroopPlacer._meshMaterial == (NativeObject)null)
                {
                    SwitchTeamOrderTroopPlacer._meshMaterial = copy.GetMeshAtIndex(0).GetMaterial().CreateCopy();
                    SwitchTeamOrderTroopPlacer._meshMaterial.SetAlphaBlendMode(Material.MBAlphaBlendMode.Factor);
                }
        ...
        public override void OnMissionScreenTick(float dt)
        {
            ...
            {
                if (this.formationDrawTimer.Check(MBCommon.GetTime(MBCommon.TimeType.Application)) && !this.IsDrawingFacing && (!this.IsDrawingForming && this.IsCursorStateGroundOrNormal() == SwitchTeamOrderTroopPlacer.CursorState.Ground) && this.GetCursorState() == SwitchTeamOrderTroopPlacer.CursorState.Ground)
        ...
        private bool IsDeployment
        {
            get
            {
                return this.Mission.GetMissionBehaviour<SiegeDeploymentHandler>() != null;
            }
        }

        private void AddAttachPoints()
        {
            foreach (Formation formation in this.PlayerTeam.FormationsIncludingSpecial.Except<Formation>(this.PlayerOrderController.SelectedFormations))
            {
